...
export function RichTextEditor({
  ...
  return (
    <div
      className={`border rounded-2xl overflow-hidden transition-all bg-bg-card duration-300 ${
        isFocused
          ? "border-brand-indigo ring-1 ring-brand-indigo"
          : "border-brand-sand/70"
      }`}
    >
      {/* Barra de Herramientas Word-Like minimalista */}
      <div className="bg-bg-base/70 border-b border-brand-sand px-3 py-2 flex flex-wrap items-center justify-between gap-2">
      ...
      <div className="relative">
        <div
          ...
          className="w-full min-h-[150px] max-h-[400px] overflow-y-auto p-4 text-xs text-text-main focus:outline-none font-sans leading-relaxed whitespace-pre-wrap select-text cursor-text"
          style={{
            minHeight: "150px",
          }}
        />
        ...
        {editorRef.current && !editorRef.current.innerHTML && (
          <div className="absolute inset-0 p-4 text-xs text-text-sub/40 pointer-events-none font-sans">
            {placeholder}
          </div>
        )}
      </div>
